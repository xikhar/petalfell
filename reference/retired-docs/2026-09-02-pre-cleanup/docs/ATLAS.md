...
transitions are derived by either production terrain path from this categorical
intent and each province's `transitionBlocks`; the source itself stays exact and
editable. The historical compiler stores continuous primary, secondary and
weight fields. The active direct runtime instead uses the same declared support
to interlock the original discrete biome surface and vegetation grammars with
broad absolute-coordinate fields; it does not mutate `region.png` or pretend the
old material palette is continuously interpolated.
...
one generic terrain-relative pass; Bloom Grove Court and Fallen Colossus are the
current promoted sites. The full atlas
...
runtime evidence. Compiler 27 remains the last complete historical mechanical
baseline. The optional `--compiled-atlas` path runs live compiler source 28 and
rebuilds stale version-27 sectors on demand; version 28 has no complete atlas
batch. Extended traversal/collision/swimming and visual quality remain unaccepted.

On 2026-09-01 the normal path gained its production vertical response. A smooth
measured transfer expands the accepted dry elevation band into Y26..166 before
the old globally anchored terrace/crown grammar and high-ground ridge fields run;
final natural terrain is capped at Y191 so the 256-block runtime retains authored
monument headroom. Representative direct windows now measure 25..46 in the
southern lowlands, 25..118 at the central river and 25..189 in northern mountain
and high-coast terrain. Low coasts use broad beach courses and visible submerged
beds; selected high wet edges use four broad displaced contour shoulders while
other runs remain sheer. Two broad low-power shader fields break the repeated
far-water read without replacing legacy translucency, depth or refraction. The
original fine ripple and sheet terms remain full strength through ordinary play
distance, then fade between 105 and 260 camera blocks so sub-pixel waves cannot
alias into one diagonal printed texture; broad world-space movement survives
that fade. Rare dry highland/snow candidates may add a globally keyed,
terrain-capped erosion arch as sparse voxels above the ordinary heightfield. A
separate mesh ceiling keeps its roof in rendering/collision without moving the
playable floor beneath its aperture.

Those broad bank, bed and shoulder courses now use a bounded 3/4 chamfer
distance. It approximates the old river/lake system's Euclidean signed edge and
removes the production path's Manhattan-diamond bands without changing accepted
wet identity or water-surface Y. Its 96-block reach remains inside the
192-block moving-window safety margin.

For a narrow inland reach, the same signed field supplies a local normal and
half-width. Its perpendicular is oriented toward lower accepted elevation, then
the original river grammar's `side * 0.10` term biases the coherent bank field.
This changes selected dry bank courses only: it does not move water, invent a
centreline or apply a directional rule to wide lakes, junctions or ocean coast.

The same normal path now treats the categorical region painting as province
intent rather than a material knife cut. A bounded per-window chamfer field uses
each province's `transitionBlocks`; broad globally keyed distance wander and
weave fields then interlock the two original discrete biome surfaces and
vegetation through that support. A nine-address wide/far matrix across the
northwest coast, northern river and summit, east confluence, dry scarp, central
lake, fen, Southreach and southeast coast was inspected after the change. The
macro silhouettes and hydrology remained intact and exact source-colour seams
became broad patches. That is agent visual evidence, not exhaustive coverage,
continuous material blending or author acceptance.

The post-transition whole-map audit rebuilt all 165 bounded windows and matched
5,984 safe terrain plus 14,208 full-overlap overhang owner comparisons across
3,072 global walking-margin chunks. All windows retained zero water-step,
severe-step and submerged-dry violations; the manifest is
`4575d8bdc98b27bd` after 415.3 seconds.

The normal-path verifier repeats, validates and hashes a bounded window and its
promoted-site/formation statistics, then builds an adjacent window and compares
the playable overlap including terrain fields, water, materials, trees and
boulders. It compares overhang voxels across the full visible intersection.
Representative mountain, high-coast, river, lowland and arch addresses pass
with zero water-step, severe-step or submerged-dry violations; the arch case
matches 771 columns/9,264 voxels across windows. Exact commands, hashes, captures
and remaining uncertainty live in
[the map-guided terrain knowledge entry](../building-knowledge/terrain/production-map-guided-terrain-runtime.md).
