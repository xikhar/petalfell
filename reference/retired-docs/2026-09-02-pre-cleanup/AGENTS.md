...
into the proven low-level system directly. Its measured elevation transfer now
spans lowlands at Y25..46, the central river window at Y25..118 and northern
mountain/high-coast windows at Y25..189, while retaining the old six-block local
terraces inside the canonical Y191 natural ceiling. Rare accepted highland/snow
candidates can add a terrain-capped sparse erosion arch while retaining the
walkable floor beneath its roof; representative moving windows now compare the
safe terrain/dressing overlap and complete visible overhang voxels exactly.
Broad beaches, submerged beds and canyon shoulders keep the accepted wet mask
and surface but measure their response with a bounded 3/4 chamfer field, matching
the old river/lake system's near-Euclidean edge instead of Manhattan diamonds.
Narrow inland reaches also recover a bounded channel normal, half-width and
downhill-oriented side directly from that accepted silhouette. This restores
the legacy `side * 0.10` broken-bank cadence without inventing a river route or
applying it to wide lakes, ocean and junctions.
The direct-runtime atlas audit has also built all 165 possible two-sector
normal-play windows: every one passed its water invariants, all 3,072 global
walking-margin chunks were covered, and 5,984 safe terrain plus 14,208 complete
overhang owner comparisons matched. This is whole-map mechanical coverage of
the active runtime, not a visual acceptance claim.
The accepted categorical region painting remains exact authored province intent,
but it no longer cuts the old surface and vegetation palettes at that pixel edge.
The direct runtime derives a bounded global chamfer field from each province's
`transitionBlocks` and uses broad stable noise to interlock the two old biome
grammars across the border. This is a discrete old-material ecotone, not shader
colour interpolation and not a mutation of `region.png`.
Dry click routes also require two blocks of clearance in the placed-voxel
overlay, so a tree trunk or authored wall cannot remain a valid waypoint merely
because the terrain surface beneath it is connected. Water remains swimmable.
Current mountain, monumental-coast, river, beach/submerged-bed, erosion-arch,
Bloom and Fallen Colossus captures have been inspected by the agent. They
establish a strong candidate and complete review boundary, not visual
acceptance: prolonged traversal, exhaustive address-by-address visual coverage, author review of
the overhang frequency/silhouette, explicit reach identity/station and
reference-level material/lighting parity remain open, and the author has not
accepted the terrain. Lowland and northern-river night wide/far captures have
also been inspected through the ordinary day cycle; that closes the former
night-water evidence gap without claiming final lighting parity. The shared
water shader preserves the exact legacy fine ripple stack through ordinary play
distance, then fades those sub-pixel terms between 105 and 260 camera blocks
while two low-power world-space fields retain broad moving variation. This
prevents the old fine ripples becoming diagonal printed wallpaper at the atlas
camera without substituting a flat far-water material.
...
1. **Review production terrain through the fast map-guided full-atlas runtime first.**
   ...
   192 sectors to judge a local terrain rule. Chapter 1's
   ...
   remains the last complete persisted evidence for the later integration gate.
   The optional `--compiled-atlas` path now runs compiler source 28 and therefore
   rebuilds stale version-27 artifacts on demand; it has not repeated the full
   192-sector gate. Do not use it as the inner visual iteration loop.
...
any in-bounds Shift-click address. A successful landing closes the map and returns
to play; rejected requests keep it open. Map pan/zoom and developer values persist.
Map travel rejects a clear but stranded shelf fragment unless its traversal
component contains at least 4,096 cells and reaches 48 blocks; low islands may
qualify through connected swimming water. Exact walking handoffs never use this
sideways recovery policy. They preserve the current transform whenever the old
and new windows have identical collision and water around the player's body;
they must not demand a fresh dry, flat teleport landing from an already-moving
player.
...
The legacy stair cut shape remains part of that low-level grammar, but its
allocation-wide connectivity planner is **legacy-fixture only**. Production
stairs must be selected from bounded, globally registered atlas candidates,
read one immutable source heightfield and merge proposals commutatively;
otherwise the same overlap can grow a stair in one 2×2-sector window and not its
neighbour. Wetland beds and saturated banks use mud rather than the legacy
world's generic wet sand, without changing accepted hydrology or the shared
water shader.

Production route-following uses cautious walk speed while the route owns input;
manual WASD remains full speed unless Shift is held. A land waypoint is reached
only when the capsule is grounded at that waypoint's own surface and inside its
cell-centre radius. Do not restore the old 1.05-block arrival threshold or permit
mid-air height crossing to consume a waypoint: both let paths skip one-block
summit stair cells and fall from otherwise valid monumental terrain.
...
./tools/world-authoring.sh capture-production-terrain 4500,1900 ../shots/north-massif atlas_play,atlas_wide
...
`verify-production-terrain` instead builds the normal map-guided old-terrain
window twice at one atlas coordinate, validates every terrain-data cell, compares
the complete repeat hash and promoted-site statistics, and resolves the same safe
landing twice. Use it for the fast terrain loop; it does not read or write a
compiler artifact.
`audit-production-terrain` builds every one of the 165 sector-aligned 2x2
windows that normal play can own. It fingerprints globally aligned chunks on
the runtime's walking-margin lattice, compares every chunk that both owners
consider playable and every visible overhang wherever owners overlap, and
requires water invariants in every window. It retains only compact fingerprints
and writes no compiler artifact; passing it is whole-map mechanical evidence,
not visual review.
`review-production-terrain` and `capture-production-terrain` open that same
normal-play builder directly at an atlas address. The capture command accepts a
comma-separated shot subset so a local visual iteration can emit only the play,
wide, far, night-wide or night-far evidence it needs. The two night shots use
the ordinary early-night `DayCycle` state rather than a review-only material or
light. These are Godot GUI commands and therefore use the workspace-5 silent
launcher.
`verify-production-playability` builds that same normal window with its real
collision chunks, playable controller and atlas water callback. `land` requires
the capsule to finish a height-changing terrace route grounded; `water` adds a
deep-water crossing and requires swimming/buoyancy to remain active.
`verify-atlas-walking-handoff` exercises cardinal, diagonal and atlas-edge
window planning plus cooldown/rearm suppression. The actual runtime transfer
compares the 3×3×5 collision/water neighbourhood around the current body in both
overlapping windows and never uses the map teleport fallback.
