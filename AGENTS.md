...
Shot names and camera framings are in `src/Tools/Capture.cs`; output lands in
`../shots/<name>/` relative to the project root. The boot log prints world
generation timings, remnant counts, footing statistics and reclamation counts —
read it, it has repeatedly exposed features that never ran.

Repo layout is in [ARCHITECTURE.md](ARCHITECTURE.md) §7.
